...
@Component({
  selector: 'app-table-zonas',
  templateUrl: './table-zonas.components.html',
  styles: ``,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class TableZonasComponents
{
  @Input() dataSource :CustomPagedResponse<zonasInterface>={};
  @Output() sendDataPagination = new EventEmitter<zonasFilter>();

  displayedColumns  : string[] = ['actions', 'idzona','idbarrio', 'descripcion'];
  ...
  editZonas(zonas: zonasInterface): void {
    const dialogRef = this.dialog.open(ModalZonasComponents, {
      width: '70%',
      maxHeight: '80vh',
      disableClose: true,
      data: { editMode: true, zonas }
    });
